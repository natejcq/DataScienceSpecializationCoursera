...
here are two logical values in R, also called boolean values. They are TRUE and FALSE. In R you can construct logical expressions which will evaluate to either TRUE or FALSE. To construct logical expressions, we make use of the logical operators. 
...
(TRUE != FALSE) == !(6==7)
```
Just like mathematical operators, we can also use parentheses to group expressions
```{r}
(FALSE == TRUE) == FALSE
```
There are two AND operators in R. The & and &&. You can use the `&` operator to evaluate AND across a vector. The `&&` version of AND only evaluates the first member of a vector.
```{r}
...
```
We can use logical operators on numeric vectors to check conditions. The result will be a logical vector 
```{r}
ints <- sample(10)
ints
ints > 5
...
```
We can then subset the list and store the specific arguments from the ellipsis into our own variables as long as the names are known. 
```{r}
mad_libs(place="Delhi", adjective="some", noun="statue")
```


### Passing a function as an argument

Below, the evaluate function will take a function and data as its arguments. It will return the result of the data when evaluated against the function.
```{r}
...
evaluate(mean, c(1,5.6,4,9))
```
It is also possible to pass functions as arguments while defining them
```{r}
evaluate(function(x){x+1},6)
```
The function passed as an argument will simply increment the value passed to it by 1
...
Dates are represented by the ```Date``` class. Times are represented by the ```POSIXct``` and ```POSIXlt``` class. Dates are stored internally as the number of days since 1970-01-01. Times are stored internally as the number of seconds since 1970-01-01. 

## Dates in R 

Dates can be coerced from strings using the ```as.Date()``` function. 
```{r}
x <- as.Date("2025-03-12")
x
unclass(x)
```
The date is printed as a string because that is how the print works. The underlying representation is that the date stored as x is 20,159 days since 1 January 1970. 

## Times in R

Times are represented via either ```POSIXct``` or ```POSIXlt```. ```POSIXct``` is a useful class when storing the times in a dataframe as it is just a very large integer. ```POSIXlt``` is a list internally and stores a lot of other useful information like the day of the week, month of the year, day of the month, day of the year, etc. 
...
unclass(p)
```
The ```strptime``` function is useful for dates that are written in a different character string format that need to be converted into date or time objects. 
```{r}
...
Addition and subtraction are the two mathematical operations that can be done on Date and Time objects. The objects need to be of the same class.
```{r}
x <- as.POSIXlt(as.Date("2012-09-01"))
y <- strptime("19 Jan 2011 11:35", "%d %b %Y %H:%M")
x-y
```
During these operations, R is cognizant of leap years, leap seconds, daylight savings and timezones.
```{r}
...
```
```{r}
t1 <- as.Date("2025-10-06")
difftime(Sys.time(), t1, units = "days")
```
We can also compare two dates / times using the relational operators.
...
* The dollar sign is a common way to access columns in a data frame. For example, if you have a data frame df and you want to access the column named sulfate, you would use: df$sulfate . However, this method requires you to know the exact name of the column at the time of writing the code. It does not work when the column name is stored in a variable.
* Using Double Brackets ([[ ]]): The double brackets allow you to access columns dynamically using a variable. For example, if you have a variable pollutant that contains the name of the column you want to access, you can do: df[[pollutant]].  This method is particularly useful when you want to write functions that can handle different column names based on user input.
```{r}
...
* If sulfate or nitrate contain NAs, cor() will return NA. &rarr; You should add use="complete.obs":
